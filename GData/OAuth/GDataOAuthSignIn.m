...
- (void)closeTheWindow;
- (void)gotTwitterPIN:(NSString*)pin;
...
- (BOOL)requestRedirectedToRequest:(NSURLRequest *)redirectedRequest {
  ...
  [self closeTheWindow];
	...
	responseStr = [[redirectedRequest URL] query];
...
- (void)gotTwitterPIN:(NSString*)pin {
  [self closeTheWindow];
	NSString *responseStr = [NSString stringWithFormat:@"oauth_verifier=%@",pin];
	  [auth_ setKeysForResponseString:responseStr];
NSLog(@"%@",responseStr);
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:accessURL_];
  [auth_ addAccessTokenHeaderToRequest:request];
...
static void ReachabilityCallBack(SCNetworkReachabilityRef target,
  ...
  GDataOAuthSignIn *signIn = (__bridge GDataOAuthSignIn *)info;
