...
- (void)signInSheetModalForWindow:(NSWindow *)parentWindowOrNil
  ...
  sheetModalForWindow_ = parentWindowOrNil;
...
- (void)webView:(WebView *)sender resource:(id)identifier didFinishLoadingFromDataSource:(WebDataSource *)dataSource {
	NSString* pin = [webView_ stringByEvaluatingJavaScriptFromString:@"document.getElementById('oauth_pin').getElementsByTagName('code')[0].innerText"];
	if([pin length]!=0 && !hasDoneFinalRedirect_) {
		hasDoneFinalRedirect_ = YES;
		[signIn_ gotTwitterPIN:pin];
	}
}
