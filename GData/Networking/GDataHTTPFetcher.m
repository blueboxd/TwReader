...
//    [self autorelease]; // in case the callback releases us
...
- (void)retryFetch {
  ...
  id holdDelegate = delegate_;//[delegate_ autorelease];
...
- (NSURLRequest *)connection:(NSURLConnection *)connection
  ...
  if (redirectRequest && redirectResponse) {
    NSMutableURLRequest *newRequest = [request_ mutableCopy];
    // copy the URL
...
- (void)setProxyCredential:(NSURLCredential *)theCredential {
  ...
  proxyCredential_ = theCredential;
}
...
- (void)setDownloadFileHandle:(NSFileHandle *)fileHandle {
  downloadFileHandle_ = fileHandle;
}
...
+ (void)setDefaultRunLoopModes:(NSArray *)modes {
  gGDataFetcherDefaultRunLoopModes = modes;
}
...
- (GDataCachedURLResponse *)cachedResponseForRequest:(NSURLRequest *)request {
  ...
  @synchronized(self) {
    NSURL *key = [request URL];
    response = [responses_ objectForKey:key];
